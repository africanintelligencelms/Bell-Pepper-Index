  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 text-slate-800 px-4 py-3 shadow-xs">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            {unlocked ? (
              ...
              /* Locked, but still visible: a farmer should be able to see the
                 tools exist, which is what makes logging worth doing. */
              <button
        ...
        {appMode === 'advanced' && !unlocked ? (
          /* The offtaker directory is open to everyone, so a farmer can be in
             advanced mode without having unlocked anything. Showing the full
             tab bar here would offer six tabs that silently bounce back. One
             locked chip instead: it still advertises what is coming — which is
             the point of earning it — without cluttering a phone screen with
             dead controls. */
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            {unlocked ? (
              ...
              <button
                onClick={() => onLockedAttempt('Market tools')}
                className="text-xs bg-slate-50 hover:bg-slate-100 text-slate-500 px-3 py-2 rounded-xl font-semibold border border-slate-200 flex items-center gap-1.5 transition"
                title="Log three sales to open the market dashboard, price history, cost calculator and chat reader."
              >
                <Lock className="w-3.5 h-3.5 text-slate-400" />
                <span>More Tools · {contributions}/3</span>
